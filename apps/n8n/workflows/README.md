...
- `career-analysis.json`: HMAC 검증, Wanted 수집·수동 원문 전달, OpenAI 2단계 구조화 분석, Worker callback을 처리하는 Workflow 원본
- `career-analysis-smoke.json`: 롤백을 위해 보존하는 비활성 초기 연결 확인 Workflow
- OpenAI 분석 노드를 실행하려면 n8n에서 `OpenAI Career Analysis` Credential을 만든 뒤 두 OpenAI 노드에 연결해야 한다.
