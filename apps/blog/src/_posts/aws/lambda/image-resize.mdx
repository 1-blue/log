...
<Blockquote type="tip">
  아래 코드는 `cloud`를 시켜서 작성한 코드고, `/images/origin/`폴더에 업로드된
  이미지가 있다면 `/images/`폴더로 이동하는 코드에요.
  <br />
  폴더구조도 그대로 가져가서 만약 `/images/origin/a/b/c/photo.jpg` 파일이 있다면
  `/images/a/b/c/photo.jpg`에 리사이징 결과물을 업로드해요.
  <br />
</Blockquote>
