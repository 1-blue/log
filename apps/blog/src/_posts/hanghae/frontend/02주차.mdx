...
<Blockquote type="tip">
  옛날에 공부용으로 `babel`을 이용해서 `jsx`를 직접 변환해본적이 있는데 그때
  변환했을때 `/*#__PURE__*/` 이런 주석이 붙어있었어요.
  <br />
  그 당시에 찾아봤을때 순수 컴포넌트임을 컴파일러한테 알려주는 역할이라고
  이해했었는데 이런것을 부르는 명칭이 있다는걸 이제 알았네요..!
  <br />
</Blockquote>
...
<Blockquote type="tip">
  여기서 `type`은 `HTML`의 `TagName`이거나 컴포넌트 함수 자체일 수 있어요.
  <br />
</Blockquote>
