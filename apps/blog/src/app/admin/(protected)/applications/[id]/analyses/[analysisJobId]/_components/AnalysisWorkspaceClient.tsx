    ...
    <ul className="grid gap-2">
      {evidence.map((item, index) => (
        ...
        >
          ...
          <p className="text-muted-foreground mt-2 break-words leading-5">
            <span className="text-foreground font-medium">문맥:</span>{" "}
            {item.context ?? "원문에서 주변 문맥을 찾지 못했습니다."}
          </p>
          <blockquote className="text-muted-foreground mt-1 break-words leading-5">
            “{item.excerpt}”
          </blockquote>
        </li>
      ))}
        ...
        <TabsContent value="summary">
          ...
          >
                ...
                <strong className="mt-1 block text-3xl">
                  {
                    workspace.questions.filter(
                      (item) => item.currentAnswer?.answer,
                    ).length
                  }
            ...
            <section className="border-border rounded-md border p-4">
              ...
              <ul className="mt-3 list-disc space-y-1 pl-5 text-sm">
                {result.comparison.applicationStrategy.keyMessages.map(
                  (item) => (
                    <li key={item}>{item}</li>
                  ),
                )}
            ...
            <dl className="text-muted-foreground grid gap-2 text-xs sm:grid-cols-2">
              ...
              <div>
                ...
                <dd className="inline font-mono">
                  {workspace.sources.jobPostingSnapshot.contentHash.slice(
                    0,
                    12,
                  )}
                  …
            ...
            <details>
              ...
              <ul className="mt-3 grid gap-2 text-xs">
                {(workspace.resultMetadata?.executions ?? []).map(
                  (execution) => (
                    <li
                      className="border-border rounded-md border p-3"
                      key={execution.step}
                    >
                      {execution.step} · {execution.model} ·{" "}
                      {execution.promptVersion} · 입력{" "}
                      {execution.inputTokens.toLocaleString()} / 출력{" "}
          ...
          <details className="border-border bg-card rounded-lg border p-5">
            ...
            <section className="mt-4 grid gap-4">
              <p className="break-words leading-7">{result.job.summary}</p>
              ...
              {[...result.job.warnings, ...result.comparison.warnings]
                .length ? (
                      ...
                      <label className="grid gap-1 text-sm font-medium">
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            {Object.entries(MATCH_LABELS).map(
                              ([value, label]) => (
                ...
                <div className="mt-4 grid gap-3">
                  ...
                  {question.answerOutline ? (
                    <div className="border-primary/20 bg-primary/5 rounded-md border p-3 text-sm">
                      ...
                      <p className="text-muted-foreground mt-1 whitespace-pre-wrap leading-6">
                        {question.answerOutline}
                      </p>
                    </div>
                  ) : null}
                  {question.modelAnswer ? (
                    ...
                    <p className="text-muted-foreground text-xs">
                      확인된 경험을 바탕으로 한 답변 초안이 없습니다. 내 경험을
                      직접 정리해 주세요.
                    </p>
                  )}
                  <label className="grid gap-1 text-sm font-medium">
              ...
              {workspace.checklist.map((item, index) => (
                ...
                >
                  ...
                  <Button
                    aria-label="아래로 이동"
                    disabled={
                      disabled || index === workspace.checklist.length - 1
                    }
                    onClick={() => void moveChecklist(index, 1)}
                    size="icon"
                    type="button"
          ...
          {showInterviewReview ? (
                ...
                {workspace.interviewNotes.map((note) => (
                  ...
                  >
                    <summary className="cursor-pointer font-medium">
                      {note.roundLabel} ·{" "}
                      {formatApplicationDate(note.interviewedAt)}
                    </summary>
                    <div className="mt-4 grid gap-3">
                      <InterviewNoteForm
                        disabled={disabled}
            ...
            <div className="border-border bg-muted/30 rounded-lg border p-5 text-sm">
              ...
              <p className="text-muted-foreground mt-1">
                면접 일정이 등록되거나 지원 상태가 면접 단계가 되면 회고를
                작성할 수 있습니다.
              </p>
            </div>
          )}
                  ...
                  <SelectContent>
                    ...
                    {workspace.history
                      ...
                      .map((item) => (
                        ...
                        >
                          {formatApplicationDate(item.completedAt)} ·{" "}
                          {item.fitScore}점
                        </SelectItem>
                      ))}
                  </SelectContent>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="bg-muted/30 rounded-md p-4">
                    ...
                    <p className="text-muted-foreground mt-2 text-xs leading-5">
                      ...
                      <br />
                      <span className="font-mono">
                        {workspace.comparison.sources.resume.contentHash.slice(
                          0,
                          8,
                        )}
                        … /{" "}
