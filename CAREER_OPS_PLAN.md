...
- [x] 응답 스키마 불일치는 자동 반복하지 않고 최종 실패·수동 재시도로 처리
...
- [x] 작업 상태 머신과 허용 전이 검증
- [x] n8n → Worker 콜백 timestamp/HMAC 검증
- [x] 단계별 heartbeat와 `last_error_code` 저장
- [x] 네트워크/429/5xx만 제한된 backoff와 jitter로 재시도
- [x] 인증/검증/지원하지 않는 공고 오류는 자동 재시도하지 않음
- [x] 부분 결과의 원자적 저장 또는 revision 규칙 구현
- [x] 동일 callback event ID 중복 처리 방지
- [x] 관리자 수동 재시도와 취소 동작 구현
- [x] 오래 멈춘 작업을 탐지하는 기준 정의

종료 기준: 새로고침·중복 클릭·일시 장애에도 작업이 중복 생성되지 않고 최종 상태가 일관된다.

- 전체 분석 작업은 같은 `analysis_jobs.id`에서 최대 2회 실행한다. 실패한 작업만 관리자가 한 번 재시도할 수 있고, 단계별 OpenAI 호출도 실행 회차마다 최대 2번으로 제한한다.
- `analysis_job_events`에 실행 회차, 단계, 단계 시도 횟수, heartbeat, 재시도 예정 시각과 오류를 불변 이력으로 저장한다. callback event ID 중복과 이전 실행에서 늦게 도착한 callback은 성공 응답만 반환하고 상태를 변경하지 않는다.
- OpenAI 네트워크·timeout·일반 429·5xx만 한 번 자동 재시도한다. `Retry-After`가 60초 이하면 따르고, 없으면 2초와 결정적 jitter를 사용한다. 인증·결제·quota·입력·미완료·스키마 오류는 자동 재시도하지 않는다.
- Worker callback은 HTTP 상태를 직접 분류해 네트워크·429·5xx만 최대 3회 전송하고 4xx는 재시도하지 않는다. 모든 실패 메시지는 계약에 정의된 오류 코드와 사용자용 메시지로 제한한다.
- 취소는 외부 n8n 실행을 강제 종료하지 않는 논리적 취소다. 취소 후 도착한 heartbeat와 결과는 무시하며, 진행 중 결과를 별도 저장하지 않고 최종 성공 callback에서만 결과·실행 통계를 원자적으로 저장한다.
- Worker Cron은 5분마다 최근 heartbeat가 20분 이상 없는 활성 작업을 최대 100건씩 `WORKFLOW_STALLED` 실패로 전환한다. 전체 작업 자동 재실행은 하지 않는다.
- 관리자 상세 화면은 실행 회차·현재 단계·최근 heartbeat·재시도 시각을 표시하며, 재시도 가능한 실패에는 같은 작업 재시도 버튼을, 활성 작업에는 확인 대화상자가 있는 취소 버튼을 제공한다. 5분 polling 종료 후에는 수동 새로고침을 제공한다.
- 다른 로컬 Supabase 프로젝트와 충돌하지 않도록 이 저장소 포트를 `5532x` 대역으로 분리했다. 로컬 migration reset, schema lint와 70개 DB 테스트를 통과했으며 원격 migration 적용·Worker Cron 배포·실제 n8n Workflow 교체는 16단계로 보류한다.
